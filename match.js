...
IMPORTANT: Respondent r_000 is the live participant submitting this request. They must appear in exactly one group in your output.
...
function filterToTesterGroup(matchResult, testerId, cohort) {
  ...
  // Detect any dropped respondents (including but not limited to the tester).
  const allInputIds = cohort.map(r => r.respondent_id);
  const allOutputIds = groups.flatMap(g => g.member_ids || []);
  const dropped = allInputIds.filter(id => !allOutputIds.includes(id));
  if (dropped.length > 0) {
    console.warn(`LLM dropped ${dropped.length} respondents: ${dropped.join(', ')}`);
  }
  
  let testerGroup = groups.find((g) => (g.member_ids || []).includes(testerId));
  if (!testerGroup) {
    // LLM dropped the tester (usually a token-budget truncation). Fall back to
    // the smallest group so the frontend can still show a real result.
    console.warn(`Tester ${testerId} not found in any LLM group — assigning to smallest group`);
    testerGroup = groups.reduce((best, g) =>
      (g.member_ids || []).length < (best.member_ids || []).length ? g : best
    , groups[0]);
    if (!testerGroup) throw new Error('LLM returned no groups');
    // Inject the tester into that group so member cards render correctly.
    testerGroup = { ...testerGroup, member_ids: [testerId, ...testerGroup.member_ids] };
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const groupCount = Math.ceil(cohortSize / TARGET_GROUP_SIZE);
